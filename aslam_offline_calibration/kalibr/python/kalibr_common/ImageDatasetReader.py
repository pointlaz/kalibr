...
# Module-level, shared by BagImageDatasetReader and McapImageDatasetReader.
_UNCOMPRESS_SNAPPY = None


def _decode_image_msg(data, timestamp, cv_bridge_instance):
  """Decode a ROS image message into a grayscale uint8 ndarray.

  Accepts sensor_msgs/Image, sensor_msgs/CompressedImage, and
  mv_cameras/ImageSnappyMsg. Returns (timestamp, img_data).
  """
  global _UNCOMPRESS_SNAPPY
  if data._type == 'mv_cameras/ImageSnappyMsg':
    if _UNCOMPRESS_SNAPPY is None:
      from snappy import uncompress
      _UNCOMPRESS_SNAPPY = uncompress
    img_data = np.reshape(_UNCOMPRESS_SNAPPY(np.fromstring(
    ...
    img_data = np.array(cv_bridge_instance.compressed_imgmsg_to_cv2(data))
    ...
    if data.encoding == "16UC1" or data.encoding == "mono16":
      image_16u = np.array(cv_bridge_instance.imgmsg_to_cv2(data))
      img_data = (image_16u / 256).astype("uint8")
    elif data.encoding == "8UC1" or data.encoding == "mono8":
      img_data = np.array(cv_bridge_instance.imgmsg_to_cv2(data))
